...
mongoose.connect(process.env.MONGODB_URI || 'mongodb://localhost:27017/warehouse')
  .then(async () => {
    ...
    // Simulate the import process - NO products tracked (all duplicates scenario)
    console.log('🔧 Simulating import process with NO products tracked...');
    
    const importedProducts = []; // EMPTY - simulating all duplicates
    const results = {
    ...
    // Simulate that NO products are tracked (extreme duplicate scenario)
    ...
    // Test the FORCE create logic with empty importedProducts
    console.log(`🚀 FORCE Creating ImportReceipt with EMPTY importedProducts...`);
      ...
      if (simulatedData && simulatedData.length > 0) {
        ...
        if (importedProducts.length > 0) {
          ...
          const importReceiptData = {
            ...
            status: "created",
            importMetadata: {
              fileName: 'test-empty-import.xlsx',
              importDate: new Date(),
    ...
    if (finalCount > 0) {
      console.log(`🎉 SUCCESS: Receipt created even with empty importedProducts!`);
    } else {
      console.log(`❌ FAILED: No receipt created`);
    }
