  ...
  <div v-if="visible" class="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full z-50">
    <div class="relative top-20 mx-auto p-5 border w-11/12 md:w-3/4 lg:w-1/2 shadow-lg rounded-md bg-white">
      <div class="mt-3">
        ...
        <div v-if="action === 'approve'" class="mb-4 p-4 bg-yellow-50 border border-yellow-200 rounded-lg">
          <div class="flex">
            <svg class="w-5 h-5 text-yellow-400 mr-2" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true">
              <path fill-rule="evenodd" d="M8.257 3.099c.765-1.36 2.722-1.36 3.486 0l5.58 9.92c.75 1.334-.213 2.98-1.742 2.98H4.42c-1.53 0-2.493-1.646-1.743-2.98l5.58-9.92zM11 13a1 1 0 11-2 0 1 1 0 012 0zm-1-8a1 1 0 00-1 1v3a1 1 0 002 0V6a1 1 0 00-1-1z" clip-rule="evenodd"/>
            </svg>
            <div>
              <h4 class="text-sm font-medium text-yellow-800">Important Notice</h4>
              <p class="text-sm text-yellow-700 mt-1">Approving this export receipt will automatically deduct the product quantities from inventory. This action cannot be undone.</p>
            </div>
          </div>
        </div>
        ...
        <div class="flex justify-end space-x-3">
          ...
          <button @click="onSubmit"
            ...
            {{ isSubmittingLocal ? 'Processing...' : (action === 'approve' ? 'Approve & Update Inventory' : 'Reject') }}
          </button>
        </div>
      </div>
    </div>
...
export default {
  ...
  watch: {
    isSubmitting(val) {
      this.isSubmittingLocal = val;
    },
    visible(val) {
      if (!val) this.localComment = '';
    },
    action() {
      this.localComment = '';
    }
  ...
  computed: {
    actionTitle() {
      return this.action === 'approve' ? 'Approve' : 'Reject';
    },
  },
  methods: {
    safeAmount(a) {
