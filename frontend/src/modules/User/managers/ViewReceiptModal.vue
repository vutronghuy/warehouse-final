              ...
              <table class="min-w-full divide-y divide-gray-200">
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                  <tr v-for="detail in receipt.details" :key="detail._id">
                    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">{{ getProductName(detail.productId) }}</td>
                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ getProductSku(detail.productId) }}</td>
                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{{ detail.quantity }}</td>
                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">${{ safeAmount(getProductPrice(detail.productId)) }}</td>
                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">${{ (detail.quantity * parseFloat(getProductPrice(detail.productId) || 0)).toFixed(2) }}</td>
                  </tr>
                </tbody>
              </table>
            </div>
          ...
          <div class="border-t pt-4 space-y-4">
            ...
            <div v-if="receipt.managerReview?.reviewedBy" class="bg-blue-50 p-4 rounded-lg">
              <div class="text-sm">
                ...
                <strong class="text-blue-800">Date:</strong> {{ formatDateTime(receipt.managerReview.reviewedAt) }}<br />
                <strong class="text-blue-800">Comment:</strong> {{ receipt.managerReview.comment || 'No comment' }}
              </div>
            </div>
            ...
            <div v-if="receipt.adminApproval?.approvedBy" class="bg-green-50 p-4 rounded-lg">
              <div class="text-sm">
                ...
                <strong class="text-green-800">Date:</strong> {{ formatDateTime(receipt.adminApproval.approvedAt) }}<br />
                <strong class="text-green-800">Comment:</strong> {{ receipt.adminApproval.comment || 'No comment' }}
              </div>
            </div>
          </div>

          <!-- Actions: open review modal from view -->
          <div class="flex justify-end mt-4 space-x-3">
            <button @click="$emit('close')" class="px-4 py-2 border border-gray-300 rounded-lg text-gray-700 hover:bg-gray-50">Close</button>
            <template v-if="receipt.status === 'created'">
              <button @click="$emit('open-review', 'reject')" class="px-4 py-2 text-red-600 hover:text-red-900">Reject</button>
              <button @click="$emit('open-review', 'approve')" class="px-4 py-2 text-green-600 hover:text-green-900">Approve</button>
            </template>
            <template v-else-if="receipt.status === 'reviewed'">
              <!-- if manager allowed to escalate/view other actions, adjust here -->
              <button @click="$emit('open-review', 'approve')" class="px-4 py-2 text-green-600 hover:text-green-900">Approve</button>
            </template>
          </div>
        </div>
